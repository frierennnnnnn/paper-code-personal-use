...
def __diffCaterogical(A, XRandomInstance, XKNNj, X):
    return int(XRandomInstance[A] != XKNNj[A])
...
def RReliefF(X, y, updates='all', k=10, sigma=30, weight_track=False, categorical_features=None):
    if updates == 'all':
        m = X.shape[0]
    else:
        m = updates
    ...
    for i in range(m):
        ...
        for j in range(k):
            ...
            for A in range(X.shape[1]):
                if A in categorical_features:
                    diff = __diffCaterogical
                else:
                    diff = __diffNumeric

                N_dA[A] = N_dA[A] + diff(A, XRandomInstance, XKNN[j], X) * __distance(k, 30)[j]
                N_dCanddA[A] = N_dCanddA[A] + (np.abs(yRandomInstance - yKNN[j]) / yRange) * __distance(k, sigma)[j] * diff(A, XRandomInstance, XKNN[j], X)
